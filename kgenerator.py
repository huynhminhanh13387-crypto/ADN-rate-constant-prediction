...
def calculate_arrhenius(A, n, Ea, T_start, T_end, step, Ea_unit="J/mol"):
    """
    ...
    Ea (float): Activation energy, in the units specified by Ea_unit.
    Ea_unit (str): "J/mol" (default) or "cal/mol".
    ...
    """
    if Ea_unit not in ("J/mol", "cal/mol"):
        raise ValueError("Ea_unit must be 'J/mol' or 'cal/mol'")
    Ea_j = Ea * CAL_TO_J if Ea_unit == "cal/mol" else Ea
    ...
    for T in temperatures:
        if T <= 0:
            continue
        ln_k = np.log(A) + n * np.log(T) - Ea_j / (R_J * T)
        k = np.exp(ln_k)
        results.append({
            'Temperature (K)': T,
            'Rate Constant k': k,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate Arrhenius k from paper Table 1 (Ea in cal/mol).")
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parent / "k_corrected.xlsx",
                        help="Output .xlsx or .csv path")
    args = parser.parse_args()
    # Source: Izato & Miyake, Combustion and Flame 198 (2018), Table 1, p.224.
    # All Ea values below are cal/mol, as stated in the table caption.
    ...
    parameter_sets = [
        ...
        {"A": 5.85e11, "n": 1.07, "Ea": 35324, "description": "No. 26"},
        ...
        {"A": 3.36e10, "n": 1.08, "Ea": 4735, "description": "No. 37"},
        ...
        {"A": 1.07e12, "n": 0.34, "Ea": 36681, "description": "No. 42"},
        ...
        {"A": 398e13, "n": 0.08, "Ea": 36013, "description": "No. 50"},
        ...
        {"A": 6.65e13, "n": 0.30, "Ea": 4095, "description": "No. 60"},
    ...
    for params in parameter_sets:
        ...
        results_df['Ea (cal/mol)'] = params["Ea"]
        results_df['Ea (J/mol)'] = params["Ea"] * CAL_TO_J
    ...
    if output_filename.suffix.lower() not in (".csv", ".xlsx"):
        raise ValueError("Output filename must end in .csv or .xlsx")
    output_filename.parent.mkdir(parents=True, exist_ok=True)
    if output_filename.suffix.lower() == ".csv":
        final_df.to_csv(output_filename, index=False, encoding="utf-8-sig")
    else:
        final_df.to_excel(output_filename, index=False, engine='openpyxl')
    print(f"\nExported {len(parameter_sets)} reactions, {len(final_df)} rows to: {output_filename}")
